...
def fit_reg_glm(train: pd.DataFrame) -> "rs.GLMModel":
...
def fit_gbm(train: pd.DataFrame) -> lgb.Booster:
    print("\n[6/8] GBM - LightGBM Poisson (log-exposure offset, CV for n_estimators) ...",
          flush=True)
    t0      = time.time()
...
def fit_catboost(train: pd.DataFrame) -> "CatBoostRegressor":
    from catboost import CatBoostRegressor, Pool, cv as cb_cv
    print("\n[7/8] CatBoost - Poisson regressor (native categoricals, log-exposure offset) ...",
          flush=True)
    t0 = time.time()
...
def predict_catboost(
    ...
    return model.predict(pool)                     # no * Exposure — offset handles it

def fit_xgb(train: pd.DataFrame) -> "xgb.Booster":  # noqa: F821
    import xgboost as xgb
    print("\n[9/9] XGBoost - Poisson (log-exposure offset, CV for n_estimators) ...",
          flush=True)
    t0 = time.time()
...
def main(
    ...
    xgb_m              = fit_xgb(train)              if run_xgb         else None
    ...
    predict_fns: dict = {
        "GLM":       lambda df: predict_glm(glm_m, df), #, glm_ncols, df, ref_levels),
